...
////////
function getCatDir2($catName,$boxName, $box_id, $cat_father_id, $catId, $link=FALSE){

	global $sessId,$db,$config,$glob,$lang_folder;
	
	// get category array for cat dir
	$query = "SELECT cat_id, cat_name, cat_father_id  ,box_id FROM ".$glob['dbprefix']."CubeCart_category ORDER BY cat_id DESC";
...
{	
	...
	if($config['gdversion']>0)
	{
		...
		$imgSpambot = "<img src=\"".$path."random/verifyGD.php?esc=".$encodedSpamCode."\" alt=\"\" title=\"\" />";
			
	} 
	else 
	{
		
		$imgSpambot = "<img src=\"".$path."random/verifySTD.php?esc=".$encodedSpamCode."&amp;n=1\" alt=\"\" title=\"\" />\r\n";
		$imgSpambot .= "<img src=\"".$path."random/verifySTD.php?esc=".$encodedSpamCode."&amp;n=2\" alt=\"\" title=\"\" />\r\n";
		$imgSpambot .= "<img src=\"".$path."random/verifySTD.php?esc=".$encodedSpamCode."&amp;n=3\" alt=\"\" title=\"\" />\r\n";
		$imgSpambot .= "<img src=\"".$path."random/verifySTD.php?esc=".$encodedSpamCode."&amp;n=4\" alt=\"\" title=\"\" />\r\n";
		$imgSpambot .= "<img src=\"".$path."random/verifySTD.php?esc=".$encodedSpamCode."&amp;n=5\" alt=\"\" title=\"\" />\r\n";
...
function getCatDir3($catName, $cat_father_id, $catId, $box_id, $link=FALSE){
