...
if(isset($_POST['deliverydays'])){ 
 ...
 $basket = $cart->updateDeliver( $deliveryDays);
 }
...
$view_cart->assign("LANG_COMMENTS",$lang['front']['cart']['comments']);
...
if($_GET['act']=="cart"){
	...
	$view_cart->assign("TXT_COMMENTS",$lang['front']['cart']['comments']);
	...
	if(isset($basket['delInf'])){
		...
		$view_cart->assign("VAL_DEL_COMMENTS",$basket['delInf']['comments']);
		...
		$view_cart->assign("VAL_DEL_COMMENTS",$ccUserData[0]['comments']);
	...
	$view_cart->assign("VAL_COMMENTS",$ccUserData[0]['comments']);
...
if($_GET['act'] != "step4") {
		if($basket['deliveryDays']  == 'normal_1'){
		$view_cart->assign("VAL_SELECTED_DELIVERY_DAYS", 'selected="selected"');
		}else if($basket['deliveryDays']  == 'urgent_1'){
		$view_cart->assign("VAL_SELECTED_DELIVERY_DAYS_TEST", 'selected="selected"');
		}
		$view_cart->parse("view_cart.cart_true.shipping_day_check");
	}
if($_GET['act']=="step3") {
...
if($basket['conts'] == TRUE) {
	...
	foreach($basket['conts'] as $key => $value){
		...
		if(file_exists($GLOBALS['rootDir']."/images/uploads/thumbs/thumb_".$product[0]['image'])){
			
			$view_cart->assign("VAL_IMG_SRC",$GLOBALS['rootRel']."images/uploads/thumbs/thumb_".$product[0]['image']);
		} else {
			$view_cart->assign("VAL_IMG_SRC",$GLOBALS['rootRel']."skins/".$config['skinDir']."/styleImages/thumb_nophoto.gif");
		}
	...
	if($_GET['act']=="step4"){
		...
		$view_cart->parse("view_cart.cart_true.shipping_true");
	}
	...
	$view_cart->assign("LANG_CART_TOTAL",$lang['front']['cart']['cart_total']);
 	...
 	$CustomerId = "SELECT * FROM CubeCart_taf WHERE customer_id = '".$ccUserData[0]['customer_id']."' ";
	$CustomerEmail = $db->select($CustomerId);
	if($CustomerEmail)
	foreach($CustomerEmail as $result){
	...
	//echo $status;
		...
		if($status != '1'){
		$discount	=	'0.00' ;
		$view_cart->assign("VAL_DISCOUNT_TOTAL",priceFormat($discount));
		}else{
