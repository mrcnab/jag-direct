...
session_start();
if($_REQUEST['customer_id'] == TRUE){
$senderCustomerId				=	$_REQUEST['customer_id'];
$_SESSION['senderCustomerId']	=	$senderCustomerId ;
}else{
$_SESSION['senderCustomerId'] = $_SESSION['senderCustomerId'] ;
}

// query database
$_GET['productId'] = treatGet($_GET['productId']);
$query = "SELECT productId, productCode, quantity, name, description, overview , feature,  image, noImages, price, popularity, sale_price, stock_level, useStockLevel, digital, digitalDir, cat_name, ".$glob['dbprefix']."CubeCart_inventory.cat_id, cat_father_id, cat_image, per_ship, item_ship, item_int_ship, per_int_ship, noProducts FROM ".$glob['dbprefix']."CubeCart_inventory INNER JOIN ".$glob['dbprefix']."CubeCart_category ON ".$glob['dbprefix']."CubeCart_inventory.cat_id = ".$glob['dbprefix']."CubeCart_category.cat_id where productId = ".$db->mySQLSafe($_GET['productId']);
...
// update amount of views
	...
	$view_prod->assign("PREVIOUS_PRODUCT",$lang['front']['viewProd']['previous_product']);
	$view_prod->assign("NEXT_PRODUCT",$lang['front']['viewProd']['next_product']);
	$view_prod->assign("LANG_TELLFRIEND",$lang['front']['viewProd']['tellafriend']);
	$view_prod->assign("TXT_PRODTITLE",validHTML(substr($prodArray[0]['name'],0,40)));
	...
	if(salePrice($prodArray[0]['price'], $prodArray[0]['sale_price'])==FALSE){
		...
		$view_prod->parse("view_prod.prod_true.price_osale");
	} else 
	{
		$view_prod->assign("TXT_PRICE","<span class='txtOldPrice'>".priceFormat($prodArray[0]['price'])."</span>");
		$view_prod->parse("view_prod.prod_true.price_wsale");
	}
	...
	$view_prod->assign("LANG_SALEPRICE",$lang['front']['viewProd']['sale_price']);
	$view_prod->parse("view_prod.prod_true.sale_price");
	} 
	...
	$view_prod->assign("PRODUCT_ID_NEXT",$prodArray[0]['productId']+1);
	$view_prod->assign("PRODUCT_ID_PREVIOUS",$prodArray[0]['productId']-1);



	if(file_exists($GLOBALS['rootDir']."/images/uploads/thumbs/thumb_".$prodArray[0]['image'])){
		$view_prod->assign("IMG_SRC","images/uploads/thumbs/thumb_".$prodArray[0]['image']);
	} else {
		$view_prod->assign("IMG_SRC",$GLOBALS['rootRel']."skins/".$config['skinDir']."/styleImages/thumb_nophoto.gif");
	}
	
	
	
	if(file_exists($GLOBALS['rootDir']."/images/uploads/thumbs/thumb_".$prodArray[0]['image'])){
		$view_prod->assign("IMG_SRC_ZOOM","images/uploads/".$prodArray[0]['image']);
	} else {
		$view_prod->assign("IMG_SRC_ZOOM",$GLOBALS['rootRel']."skins/".$config['skinDir']."/styleImages/thumb_nophoto.gif");
	}
...
if(($prodArray[0]['overview'] || $prodArray[0]['feature'] == TRUE) &&  ($prodArray[0]['overview'] > 6 || $prodArray[0]['feature'] > 6)){


	$view_prod->assign("OVERVIEW_DESCRIPTION", $prodArray[0]['overview']);
	$view_prod->parse("view_prod.more_description.overview");

	

	$view_prod->assign("OVERVIEW_FEATURE", $prodArray[0]['feature']);
	$view_prod->parse("view_prod.more_description.features");


$view_prod->parse("view_prod.more_description");
}
