...
if($basket == TRUE && isset($_POST['gateway']) && !eregi("[^0-9a-z_-]",$_POST['gateway'])) {
	...
	$FriendEmail = "SELECT * FROM CubeCart_taf WHERE customer_id = '".$ccUserData[0]['customer_id']."' ";
	$myEmail = $db->select($FriendEmail);
	if($myEmail){
	foreach($myEmail as $result){
	...
	if($status == 1)  {
		...
		$recordEE["status"] = 0;
		$update = $db->update($glob['dbprefix']."CubeCart_taf", $recordEE, $where );	
	}else{
	$where = "customer_id=".$db->mySQLSafe($ccUserData[0]['customer_id']);
	$recordEE["status"] = 0;
	$update = $db->update($glob['dbprefix']."CubeCart_taf", $recordEE, $where );
