...
for ($i=0; $i<count($subCategories); $i++){
		...
		//	validHTML($GLOBALS['rootRel']."images/uploads/nophoto.gif");
				$view_cat->assign("IMG_CATEGORY",$GLOBALS['rootRel']."images/uploads/nophoto.gif");
			} else {
		...
		$view_cat->assign("PRODUCT_NAME",$lang['front']['products']['productname']);
		
		$view_cat->assign("TXT_LINK_CATID",$subCategories[$i]['cat_id']);

		$view_cat->assign("TXT_CATEGORY", validHTML(substr($subCategories[$i]['cat_name'],0,20)));
...
// get current category info
	...
	if(isset($_GET['searchStr'])){
		...
		$view_cat->assign("LANG_LATEST_PRODUCTS",$lang['front']['viewCat']['latest_products']);
		$view_cat->assign("TXT_CAT_TITLE",$lang['front']['viewCat']['search_results']);
		$view_cat->assign("PRODUCT_NAME",$lang['front']['viewCat']['productname']);
		$view_cat->assign("SIZE_CHART",$lang['front']['viewCat']['sizechart']);
...
if($productResults == TRUE){
	...
	if($_GET['catId']>0){
		...
		$view_cat->assign("CURRENT_DIR",getCatDir(validHTML($currentCat[0]['cat_name']),$currentCat[0]['cat_father_id'], $currentCat[0]['cat_id']));
	...
	for ($i=0; $i<count($productResults); $i++){
		...
		if(file_exists($GLOBALS['rootDir']."/images/uploads/".$productResults[$i]['image'])){
			...
			$view_cat->assign("SRC_PROD",$GLOBALS['rootRel']."images/uploads/".$productResults[$i]['image']);
		} else {
			$view_cat->assign("SRC_PROD",$GLOBALS['rootRel']."skins/".$config['skinDir']."/styleImages/thumb_nophoto.gif");
		}
		...
		$view_cat->parse("view_cat.productTable.products.sale_true");
		}
		
		if($salePrice == ""){
		$view_cat->assign("TXT_PRICE",priceFormat($productResults[$i]['price']));
		$view_cat->parse("view_cat.productTable.products.sale_false");
		}
