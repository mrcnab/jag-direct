...
if($latestProducts==TRUE){
	...
	$index->assign("SIZE_CHART",$lang['front']['index']['sizechart']);	
		...
		$index->parse("index.latest_prods.repeat_prods.sale_true");
		}
		
		if($salePrice == ""){
		$index->assign("TXT_PRICE",priceFormat($latestProducts[$i]['price']));
		$index->parse("index.latest_prods.repeat_prods.sale_false");
		}
		
	$index->assign("VAL_WIDTH", $config['gdthumbSize']+75);		
	$index->assign("VAL_PRODUCT_ID",$latestProducts[$i]['productId']);	
	$index->assign("VAL_PRODUCT_NAME",validHTML(substr($latestProducts[$i]['name'],0,12)));	
...
if($specialProducts==TRUE){	
	for($j=0;$j<count($specialProducts);$j++){
		...
		$salePrice = salePrice($specialProducts[$j]['price'], $specialProducts[$j]['sale_price']);
		
		if($salePrice != ""){
		$index->assign("TXT_SALE_SPECIAL_PRICE", priceFormat($salePrice));	
		$index->assign("TXT_SPECIAL_PRICE",priceFormat($specialProducts[$j]['price']));
		$index->parse("index.mobile_accessories.repeat.sale_true");
		}
		
		if($salePrice == ""){
		$index->assign("TXT_SPECIAL_PRICE",priceFormat($specialProducts[$j]['price']));
		$index->parse("index.mobile_accessories.repeat.sale_false");
		}
