...
if(isset($_POST['email'])){
	...
	$errorMsg = $lang['front']['reg']['email_in_use'];
		...
		if($_SESSION['senderCustomerId'] == TRUE ){
		
		$FriendEmail = "SELECT MAX( customer_id ) from CubeCart_customer";
		$myEmail = $db->select($FriendEmail);
		$lastestId	=  $myEmail[0]['MAX( customer_id )'] ;
		$_SESSION['lastInsertedId']	=	$lastestId ;
		
		$record1["customer_id"] = $db->mySQLSafe($_SESSION['senderCustomerId']);
		$record1["status"] = $db->mySQLSafe('1');
		$insert = $db->insert($glob['dbprefix']."CubeCart_taf", $record1);
		
		$record2["customer_id"] = $db->mySQLSafe($lastestId);
		$record2["status"] = $db->mySQLSafe('1');
		$insert = $db->insert($glob['dbprefix']."CubeCart_taf", $record2);
		}	
