...
$box_content = new XTemplate ("skins/".$config['skinDir']."/styleTemplates/boxes/session2.tpl");
...
if($ccUserData[0]['customer_id']>0){
		...
		$box_content->assign("LANG_YOUR_ACCOUNT",substr($lang['front']['boxes']['ur_account'],0,13));
		$box_content->parse("session2.session2_true");
		...
		$box_content->parse("session2.session2_false");

}

$box_content->parse("session2");
$box_content = $box_content->text("session2");
?>