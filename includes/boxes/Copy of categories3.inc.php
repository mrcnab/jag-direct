		...
		for ($i=0; $i<count($sub_result_box); $i++){
			...
			if($sub_result == TRUE){
				for ($j=0; $j<count($sub_result); $j++){
					...
					$box_content->parse("categories3.boxcategory.maincategory.subcategory");
				}
				$box_content->parse("categories3.boxcategory.maincategory");
			}
			$box_content->parse("categories3.boxcategory.category");		
			} // end for loop
			}			
