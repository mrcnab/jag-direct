...
// query database
$results = $db->select("SELECT id, imagePath, imageName FROM ".$glob['dbprefix']."CubeCart_banner2 WHERE status = 1 and loc = 0 ORDER BY id desc");

$box_content=new XTemplate ("skins/".$config['skinDir']."/styleTemplates/boxes/bannerLeft.tpl");
	// loop results
	if($results == TRUE){	
				...
				foreach($results as $value){
					...
					$box_content->parse("bannerLeft");
				}
			}	
	}
	

$box_content = $box_content->text("bannerLeft");
?>