...
$box_content = new XTemplate ("skins/".$config['skinDir']."/styleTemplates/boxes/siteDocsFooter.tpl");
...
if($results == TRUE){
	...
	for ($i=0; $i<count($results); $i++){
		...
		if($i<count($results)-1){
			$box_content->parse("site_docs_footer.a.sep");
		}
	...
	$results[$i]['doc_link'] = "index.php?act=viewDoc&amp;docId=".$results[$i]['doc_id'];	
		...
		$box_content->parse("site_docs_footer.a");
	
	} // end loop 
}
$box_content->parse("site_docs_footer");
$box_content = $box_content->text("site_docs_footer");
?>