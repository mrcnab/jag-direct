...
if(isset($_GET["delete"]) && $_GET["delete"]>0){
	...
	$record["box_id"] = $db->mySQLSafe($_POST['box_id']);
...
<table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
  ...
  <tr>
    ...
    <td nowrap='nowrap'  class="<?php echo $cellColor; ?>"><span class="txtDir"><?php echo getCatDir3($results[$i]['cat_name'],$results[$i]['cat_father_id'], $results[$i]['cat_id'], $results[$i]['box_id']);?></span></td>
	...
	?>
	<img src="<?php echo $glob['rootRel'];?>images/uploads/thumbs/thumb_<?php echo $results[$i]['cat_image']; ?>" alt="<?php echo $results[$i]['cat_name']; ?>" title="" <?php if($imgSize['0']>49){ ?>height="50"<?php } // end if image exists ?> />
	...
	href="?edit=<?php echo $results[$i]['cat_id']; ?>&box_id=<?php echo $results[$i]['box_id']; ?>" class="txtLink"
...
<table border="0" cellspacing="0" cellpadding="3" class="mainTable">
  ...
  <tr>
    <td class="tdText">Select Box:</td>
    <td>
	
    <?php
	$db = new db();
	$query = "SELECT box_id, box_title FROM ".$glob['dbprefix']."CubeCart_boxs ORDER BY box_title DESC";
	$boxes = $db->select($query);
	?>
	<select name="box_id" id="box_id" class="textbox">
	<option value="0">Select box for category</option>
	<?php 
  	for ($i=0; $i<count($boxes); $i++){
	 $box_id	=	$boxes[$i]['box_id'];
 ?>
  <option <? if($_REQUEST['box_id']== $box_id){ ?> selected="selected" <? } ?> value="<?=$box_id?>"><?php echo $boxes[$i]['box_title']; ?>	</option>
	<?php
    ...
    <td valign="top">
       <div id="selectedImage">
    	...
    	<div id="naveed">
        	...
        	<?php if(!empty($results[0]['cat_image'])){ ?><img src="<?php echo $GLOBALS['rootRel']."images/uploads/thumbs/thumb_".$results[0]['cat_image']; ?>" alt="<?php echo $results[0]['cat_image']; ?>" title="" />	
  ...
  <tr>
    ...
    <td><input name="upload" style="width: 200px;" class="submit" type="button" id="upload" onclick="openPopUp('../filemanager/upload.php?custom=1','scrollbars=yes','filemanager',900,800)" value="<?php echo $lang['admin']['categories']['upload_new_image']; ?>" /></td>
