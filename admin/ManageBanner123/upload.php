...
if($_FILES['FCKeditor_File']['size'] > $config['maxImageUploadSize']) { 
	...
	$config['gdmaxImgSize'] = 436; 
	$config['gdthumbSize'] = 436; 
	if (is_uploaded_file($_FILES['FCKeditor_File']['tmp_name'])) {
		echo $savefile = $GLOBALS['rootDir']."/images/uploads/".$uploadFileName;
	...
	echo $imageFormat = strtoupper(ereg_replace(".*\.(.*)$","\\1",$uploadFileName));
