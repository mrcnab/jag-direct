...
<form action="<?php echo $GLOBALS['rootRel'];?>admin/ManageBanner/index.php" method="post" enctype="multipart/form-data" target="_self">
  <table align="center" border="0" cellpadding="3" cellspacing="0" class="mainTable">
	...
	<tr>
	<td class="tdTitle">Banner Location</td>
	</tr>
	<tr>
      <td> <select name="loc" style="width:200px;">
  ...
  <TABLE>
    <tr>
      <td><span class="copyText">
        <?php
				if($_POST){
		echo $uploadFileName = str_replace(array(" ","%20"),"_",$_FILES['FCKeditor_File']['name']);
...
?>
	...
	<table style="border:1px solid #333333; width:100%;" align="center">
		...
		<tbody>
		<?php
			$query = "SELECT * FROM ".$glob['dbprefix']."CubeCart_banner2 WHERE isGallery=0 ORDER BY id ASC";
			$results = $db->select($query);
			...
			if(count($results) > 0) {	
				...
				?>		
					<tr>
						...
						<td  align="center" ><div  align="center" style="width:300px; overflow:hidden;"><img src="../../BannerImages/thumbs/thumb_<?=$results[$i]['imagePath']?>" /></div></td>
						...
						<td  align="center"><a href="<?php echo $GLOBALS['rootRel']; ?>admin/ManageBanner/index.php?loc=<? if($results[$i]['loc']==1)echo 0; else echo 1;?>&amp;id=<?=$results[$i]['id']?>" class="submit"><?php if($results[$i]['loc']==1) echo "RIGHT"; else echo "LEFT";?></a></td>
