...
// generate product code
	...
	$record["overview"] = $db->mySQLSafe($_POST['FCKeditor1']);
	
	$record["feature"] = $db->mySQLSafe($_POST['FCKeditor2']);
	...
	$record["bestAccessories"] = $db->mySQLSafe($_POST['bestAccessories']); 
...
if(!isset($_GET['mode'])){
	...
	$query = "SELECT cat_id, cat_name, cat_father_id, box_id FROM ".$glob['dbprefix']."CubeCart_category ORDER BY box_id DESC";
...
<table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
  ...
  <tr>
    ...
    <td class="<?php echo $cellColor; ?>"><span class="txtDir"><?php echo getCatDir3($results[$i]['cat_name'],$results[$i]['cat_father_id'], $results[$i]['cat_id'], $results[$i]['box_id']);?></span><br />
...
<tr>
    ...
    <td colspan="2" class="tdRichText"><span class="tdText"><strong><?php echo $lang['admin']['products']['overview'];?></strong> <?php echo $lang['admin']['products']['overview_lang'];?></span>
...
<?php
		...
		if(isset($results[0]['overview'])){ 
			$oFCKeditor1->Value = $results[0]['overview'];
...
<tr>
    ...
    <td colspan="2" class="tdRichText"><span class="tdText"><strong><?php echo $lang['admin']['products']['feature'];?></strong> <?php echo $lang['admin']['products']['overview_lang'];?></span>
...
<?php
		...
		if(isset($results[0]['feature'])){ 

			$oFCKeditor2->Value = $results[0]['feature'];
...
?>
  ...
  <tr>
	...
	<?php for ($i=0; $i<count($categoryArray); $i++){ ?>
	<?php 
	//	$box_id	=	$categoryArray[$i]['box_id'] ;
	//  get box array for cat dir
	//	$Boxquery = "SELECT box_title FROM ".$glob['dbprefix']."CubeCart_boxs WHERE box_id =".$box_id ;
	// 	$boxArray = $db->select($Boxquery);
	//	print_r($boxArray[0]['box_title']) ;
	 ?>
	<option value="<?php echo $categoryArray[$i]['cat_id']; ?>" <?php if(isset($results[0]['cat_id']) && $categoryArray[$i]['cat_id']==$results[0]['cat_id']) { echo "selected='selected'"; } ?>>
	
	<?php echo getCatDir3($categoryArray[$i]['cat_name'],$categoryArray[$i]['cat_father_id'], $categoryArray[$i]['cat_id'],  $categoryArray[$i]['box_id']); ?>
  ...
  <tr>
	...
	<?php if(!empty($results[0]['image'])){ ?><img src="<?php echo $GLOBALS['rootRel']."images/uploads/".$results[0]['image']; ?>" alt="<?php echo $results[0]['image']; ?>" title="" />
  ...
  <tr>
    ...
    <td><input name="upload" style="width: 200px;" class="submit" type="button" id="upload" onclick="openPopUp('../filemanager/upload.php?custom=1','filemanager',900,800)" value="<?php echo $lang['admin']['products']['upload_new'];?>" /></td>

  </tr>

  <tr>

    <td><input name="browse" style="width: 200px;" class="submit" type="button" id="browse" onclick="openPopUp('../filemanager/browse.php?custom=1','filemanager',450,250)" value="<?php echo $lang['admin']['products']['browse_existing'];?>" /></td>
...
<input name="useStockLevel" type="radio" value="0" <?php if(isset($results[0]['useStockLevel']) && $results[0]['useStockLevel']==0) echo "checked='checked'"; ?> /></td>
  ...
  <tr>
    ...
    <td class="tdText"><strong><?php echo $lang['admin']['products']['mobile_accessories'];?></strong></td>
 ...
 <option value="0" <?php if(isset($results[0]['showFeatured']) && $results[0]['showFeatured']==0) echo "selected='selected'"; ?>><?php echo $lang['admin']['no'];?></option>
   ...
   <tr>
    ...
    <td class="tdText"><strong>Best Price Accessories</strong></td>

    <td class="tdText">

	<select name="bestAccessories" class="textbox">

 <option value="0" <?php if(isset($results[0]['bestAccessories']) && $results[0]['bestAccessories']==0) echo "selected='selected'"; ?>><?php echo $lang['admin']['no'];?></option>

        <option value="1" <?php if(isset($results[0]['bestAccessories']) && $results[0]['bestAccessories']==1) echo "selected='selected'"; ?>><?php echo $lang['admin']['yes'];?></option>
